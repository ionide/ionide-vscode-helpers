...
module Protocol =
    type [<AllowNullLiteral>] [<Import("ProtocolServer","protocol")>] ProtocolServer() =
        ...
        member __.start(inStream: NodeJS.ReadableStream, outStream: NodeJS.WritableStream): unit = jsNative
        ...
        member __.sendRequest(command: string, args: obj, timeout: float, cb: Func<DebugProtocol.Response, unit>): unit = jsNative
...
module DebugSession =
    ...
    and [<AllowNullLiteral>] [<Import("DebugSession","vscode-debugadapter")>] DebugSession(?obsolete_debuggerLinesAndColumnsStartAt1: bool, ?obsolete_isServer: bool) =
        ...
        member __.runInTerminalRequest(args: DebugProtocol.RunInTerminalRequestArguments, timeout: float, cb: Func<DebugProtocol.RunInTerminalResponse, unit>): unit = jsNative
        member __.dispatchRequest(request: DebugProtocol.Request): unit = jsNative
