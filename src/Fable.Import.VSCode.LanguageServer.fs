...
// ts2fable 0.0.0
    module Client =
        ...
        type [<RequireQualifiedAccess>] ErrorAction =
            | Continue = 1
            | Shutdown = 2

        type [<RequireQualifiedAccess>] CloseAction =
            | DoNotRestart = 1
            | Restart = 2
        ...
        type [<RequireQualifiedAccess>] RevealOutputChannelOn =
            | Info = 1
            | Warn = 2
            | Error = 3
            | Never = 4

        type [<AllowNullLiteral>] Middleware =
            interface end
        ...
        type [<AllowNullLiteral>] StateChangeEvent =
            abstract oldState: State with get, set
            abstract newState: State with get, set

        type [<AllowNullLiteral>] RegistrationData<'T> =
        ...
        type [<AllowNullLiteral>] StaticFeature =
        ...
        type [<AllowNullLiteral>] DynamicFeature<'T> =
        ...
        type [<AllowNullLiteral>] MessageTransports =
        ...
        type [<AllowNullLiteral>] BaseLanguageClient =
    ...
    type [<AllowNullLiteral>] Executable =
    ...
    type [<RequireQualifiedAccess>] TransportKind =
        | Stdio = 0
        | Ipc = 1
        | Pipe = 2
        | Socket = 3
    ...
    type [<AllowNullLiteral>] StreamInfo =
        abstract writer: obj with get, set
        abstract reader: obj with get, set
        abstract detached: bool option with get, set

    type [<AllowNullLiteral>] ChildProcessInfo =
