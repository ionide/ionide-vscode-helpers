...
module rec ws =
    ...
    module WebSocket =
        ...
        and [<AllowNullLiteral>] [<Import("WebSocket.Server","ws")>] Server(?options: IServerOptions, ?callback: Function) =
            ...
            [<Emit("$0.on('error',$1...)")>] member __.on_error(cb: (Error -> unit)): obj = jsNative
            ...
            [<Emit("$0.addListener('error',$1...)")>] member __.addListener_error(cb: (Error -> unit)): obj = jsNative
    ...
    type [<AllowNullLiteral>] [<Import("default","ws")>] WebSocket(address: string, ?protocols: U2<string, ResizeArray<string>>, ?options: WebSocket.IClientOptions) =
        ...
        member __.onerror with get(): (Error -> unit) = jsNative and set(v: (Error -> unit)): unit = jsNative
        ...
        member __.send(data: obj, ?cb: (Error -> unit)): unit = jsNative
        member __.send(data: obj, options: obj, ?cb: (Error -> unit)): unit = jsNative
        member __.stream(options: obj, ?cb: (Error -> bool -> unit)): unit = jsNative
        member __.stream(?cb: (Error -> bool -> unit)): unit = jsNative
        ...
        [<Emit("$0.addEventListener('error',$1...)")>] member __.addEventListener_error(?cb: (Error -> unit)): unit = jsNative
